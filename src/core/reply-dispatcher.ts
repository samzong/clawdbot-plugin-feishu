...
import { getRuntime } from "../plugin/index.js";
...
export function createReplyDispatcher(params: CreateReplyDispatcherParams) {
    ...
    const typingCallbacks = createTypingCallbacks({
        start: async () => {
            ...
            try {
                const reactionId = await addReaction(feishuCfg, { messageId: replyToMessageId, emojiType: Emoji.EYES });
        ...
        stop: async () => {
            ...
            try {
                await removeReaction(feishuCfg, { messageId: typingState.messageId, reactionId: typingState.emoji });
                typingState = null;
